...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        xyz_veh = np.ones((4,1))
        xyz_veh[0:3] = x[0:3]
        xyz_sens = self.veh_to_sens * xyz_veh
        
        # Calculate the angle of the object in sensor coordinates
        angle = np.arctan(xyz_sens[1]/xyz_sens[0])
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            if x[0]==0:
                raise ValueError("Division by zero in camera model.")
