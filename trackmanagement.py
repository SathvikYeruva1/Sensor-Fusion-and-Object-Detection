...
class Track:
    ...
    def __init__(self, meas, id):
        ...
        # Transformation from sensor to vehicle coordinates
        M_trans = meas.sensor.sens_to_veh[0:3, 3]
        xyz_hom = np.ones((4,1))
        xyz_hom[0:3] = meas.z[0:3] 
        xyz_veh = meas.sensor.sens_to_veh * xyz_hom

        # Initialize state vector x
        self.x = np.zeros((6,1))
        self.x[0:3] = xyz_veh[0:3]

        # Initialize covariance matrix P
        P_pos = M_rot * meas.R * np.transpose(M_rot)
        sigma_p44, sigma_p55, sigma_p66 = 50, 50, 5  # Initial settings
